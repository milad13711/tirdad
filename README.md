...
`LIMO_API_KEY` / `LIMO_PATTERN_ID` را تکمیل کنید — **پیش از استفاده در پروداکشن
حتماً `src/lib/sms/limo.ts` را طبق مستندات پنل خودتان بازبینی کنید** (مستندات
API لیمو در زمان ساخت این پروژه در برابر دسترسی خودکار ۴۰۳ برمی‌گرداند، پس شکل
دقیق endpoint/پارامترها تاییدنشده است).
